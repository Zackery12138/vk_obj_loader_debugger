...
#		define ASSETDIR_ "assets/cw1/"
		constexpr char const* kObjPath = ASSETDIR_"sponza_with_ship.obj";
#		undef ASSETDIR_
...
int main() try
{
	...
	lut::Sampler defaultSampler = lut::create_default_sampler(window);
	{
		...
		SimpleModel aModel = load_simple_wavefront_obj(cfg::kObjPath);
		ourModel = set_up_model(window, allocator, aModel, loadCmdPool.handle, dPool.handle, defaultSampler.handle, objectLayout.handle);
	}
	...
	while (!glfwWindowShouldClose(window.window))
	{
		...
		record_commands(cbuffers[imageIndex], renderPass.handle, framebuffers[imageIndex].handle, pipe.handle,
			window.swapchainExtent, sceneUBO.buffer, sceneUniforms, pipeLayout.handle, sceneDescriptors, ourModel);
...
namespace
{
	void update_scene_uniforms(glsl::SceneUniform& aSceneUniforms, std::uint32_t aFramebufferWidth, std::uint32_t aFramebufferHeight, UserState aState)
	{
		...
		aSceneUniforms.camera *= glm::translate(glm::vec3(-3.f, -5.0f, 3.f)); //move camera to appropriate position in scene
...
namespace
{
	...
	lut::Pipeline create_pipeline(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout)
	{
		...
		vertexAttribs[2].offset = sizeof(glm::vec3) * 2;
	...
	void record_commands(VkCommandBuffer aCmdBuff, VkRenderPass aRenderPass, VkFramebuffer aFramebuffer,
		...
		VkPipelineLayout aGraphicsLayout, VkDescriptorSet aSceneDescriptors, ModelPack& aModel)
	{
	   //Begin recording commands
		...
		{
			if (mesh.materialDescriptor != VK_NULL_HANDLE)
			{
				vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aGraphicsLayout, 1, 1, &mesh.materialDescriptor, 0, nullptr);
			}
